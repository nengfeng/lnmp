...
if [[ "${Platform}" =~ ^debian$|^deepin$|^kali$ ]]; then
  ...
  if [[ "${Platform}" =~ ^deepin$ ]]; then
    [[ "${Debian_ver}" =~ ^20$ ]] && Debian_ver=10 || true
    [[ "${Debian_ver}" =~ ^23$ ]] && Debian_ver=11 || true
  elif [[ "${Platform}" =~ ^kali$ ]]; then
    [[ "${Debian_ver}" =~ ^202 ]] && Debian_ver=10 || true
  fi
  ...
  if [[ "${Platform}" =~ ^linuxmint$ ]]; then
    [[ "${VERSION_MAIN_ID}" =~ ^18$ ]] && Ubuntu_ver=16 || true
    [[ "${VERSION_MAIN_ID}" =~ ^19$ ]] && Ubuntu_ver=18 || true
    [[ "${VERSION_MAIN_ID}" =~ ^20$ ]] && Ubuntu_ver=20 || true
    [[ "${VERSION_MAIN_ID}" =~ ^21$ ]] && Ubuntu_ver=22 || true
  elif [[ "${Platform}" =~ ^elementary$ ]]; then
    [[ "${VERSION_MAIN_ID}" =~ ^5$ ]] && Ubuntu_ver=18 || true
    [[ "${VERSION_MAIN_ID}" =~ ^6$ ]] && Ubuntu_ver=20 || true
    [[ "${VERSION_MAIN_ID}" =~ ^7$ ]] && Ubuntu_ver=22 || true
  fi
else
...
# Check OS Version
if [ "${Debian_ver:-0}" -lt 9 ] 2>/dev/null && [ "${Family}" = "debian" ]; then
  die_hard "Does not support this OS, Please install Debian 9+,Ubuntu 16+"
fi
if [ "${Ubuntu_ver:-0}" -lt 16 ] 2>/dev/null && [ "${Family}" = "ubuntu" ]; then
  die_hard "Does not support this OS, Please install Debian 9+,Ubuntu 16+"
fi
...
[ ${gcc_ver} -lt 5 ] && redis_ver=6.2.14 || true
...
if [ "${Debian_ver:-0}" -ge 9 ] 2>/dev/null || [ "${Ubuntu_ver:-0}" -ge 16 ] 2>/dev/null; then
  sslLibVer=ssl102
else
  sslLibVer=unknown
fi

[ -e ~/.oneinstack ] && /bin/mv ~/.oneinstack ~/.lnmp || true
